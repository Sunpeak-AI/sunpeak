...
Use `sunpeak dev --prod-mcp` to test production behavior without deploying:

```bash
sunpeak build && sunpeak dev --prod-mcp
```

This runs the dev simulator alongside a production-like MCP server that uses real handlers instead of simulation fixtures.
