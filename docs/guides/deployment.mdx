...
**Stateless mode (recommended for serverless)** — Set `stateless: true` to disable session tracking entirely. Each request creates a fresh server instance, so any backend can handle any request:
