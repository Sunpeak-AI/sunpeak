...
|----------|---------|-------------|
| `SUNPEAK_LIVE_BROWSER` | auto-detected | Browser to import session from (`chrome`, `arc`, `brave`, `edge`) |
...
<AccordionGroup>
  <Accordion title="'Not logged into ChatGPT' error">
    On first run, a browser window opens for you to log in to ChatGPT. The session is saved to `tests/live/.auth/chatgpt.json` and reused for 24 hours. If the error persists, delete the `.auth/` directory and run `pnpm test:live` again to re-login.
  </Accordion>
