...
The model never sees this tool. The resource component calls it directly with [`useCallServerTool`](/api-reference/hooks/use-call-server-tool).
...
Serve binary content (video, PDF) via MCP resources and fetch it with [`useReadServerResource`](/api-reference/hooks/use-read-server-resource).
...
Use [`useToolData`](/api-reference/hooks/use-tool-data)'s `inputPartial` to show a preview while the LLM is still generating tool arguments. This lowers perceived latency for tools with large inputs like code or structured data.
...
When a runtime error occurs in your resource (API failure, permission denied, resource unavailable), use [`useUpdateModelContext`](/api-reference/hooks/use-update-model-context) to inform the model:
...
Use [`useRequestDisplayMode`](/api-reference/hooks/use-request-display-mode) to toggle fullscreen. Always check `availableModes` first — not all hosts support all modes.
...
[`useAppState`](/api-reference/hooks/use-app-state) syncs React state to the model context automatically after each update. Best for simple state that the model should always see.
...
[`useUpdateModelContext`](/api-reference/hooks/use-update-model-context) gives you full control over what and when to send. Best for large or structured context, or when you want to batch updates.
...
<Info>
`updateModelContext` is deferred — the model sees the updated context on its next turn, not immediately. It does not trigger a model response. Use [`useSendMessage`](/api-reference/hooks/use-send-message) to trigger a response.
</Info>
