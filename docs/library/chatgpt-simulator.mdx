---
title: "Simulator"
description: "Local development environment replicating the MCP Apps host runtime."
---
...
The simulator is a local development environment that replicates the MCP Apps host runtime (ChatGPT, Claude, etc.). It uses the `ChatGPTSimulator` React component.

## Why Use the Simulator?

<Card horizontal title="Instant Feedback" icon="rotate">
  See changes immediately with hot module replacement. No more constantly reloading the host.
</Card>
...
<Tabs>
  ...
  <Tab title="Library (TypeScript)">
    ...
    const simulations: Record<string, Simulation> = {
      'albums-show': {
        name: 'albums-show',
        resourceUrl: 'http://localhost:5173/resources/albums',
