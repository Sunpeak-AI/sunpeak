...
<Tip>
Your project's `tests/e2e/helpers.ts` wraps `createInspectorUrl` with `devOverlay: false` so the dev timing overlay doesn't interfere with element assertions. Import from `./helpers` instead of `sunpeak/chatgpt` in your e2e tests.
</Tip>
