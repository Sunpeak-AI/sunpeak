...
keywords: ["MCP app testing", "ChatGPT app testing", "MCP Apps", "ChatGPT Apps", "Playwright", "e2e testing", "unit testing", "live testing", "evals", "LLM testing"]
---
...
3. **Evals** against multiple LLM models — `sunpeak/eval` connects to any MCP server, discovers tools via MCP protocol, sends prompts to multiple models (GPT-4o, Claude, Gemini, etc.), and asserts that each model calls the right tools with the right arguments. Each eval runs multiple times per model to measure reliability.

4. **Unit tests** — Vitest with happy-dom for component and hook logic testing.
...
For sunpeak projects, the dev server starts automatically when you run evals. For non-sunpeak projects, make sure your MCP server is running or set the `server` option in the config.
