...
|---------|--------------|---------|
| `sunpeak test` | E2E tests against the inspector | Playwright + inspector |
| `sunpeak test --unit` | Unit tests (Vitest) | happy-dom |
...
```bash
sunpeak test                               # Run all
sunpeak test -- --ui                       # Playwright UI mode
sunpeak test tests/e2e/albums.spec.ts      # Single file
```
