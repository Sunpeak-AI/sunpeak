...
```
  ...
  <Step title="Create resource, tool, and simulation files">
    ...
    export default async function (_args: Record<string, unknown>, _extra: ToolHandlerExtra) {
      return { structuredContent: { message: 'Hello from handler!' } };
    }
    ```
