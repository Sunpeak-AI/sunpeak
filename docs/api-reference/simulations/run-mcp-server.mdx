...
<Tabs>
  ...
  <Tab title="Library">
    ...
    import { resource } from '../src/resources/example/example';
    import exampleSimulation from '../tests/simulations/show-example.json';
...
The server creates MCP Tools and Resources from your tool files and simulations:

1. **Tools** - One per tool file, using the `tool` and `schema` exports. When called in dev mode, the real handler runs. When called via simulations, returns `toolResult.structuredContent`. Tool metadata including `visibility` is preserved.
2. **Resources** - One per resource, containing the built resource HTML from `distPath`. Resource metadata including `permissions`, `csp`, `prefersBorder`, and `domain` is preserved.
