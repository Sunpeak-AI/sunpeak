...
<Tabs>
  <Tab title="Framework (JSON)">
    In the sunpeak framework, simulations are **JSON files** that live in the `tests/simulations/` directory. The resource component and metadata are automatically linked based on directory structure.
    ...
    Resource files live separately in `src/resources/{name}/`:
...
```typescript
{
  _meta: {
    ...
    'openai/widgetCSP': {
      connect_domains: [],
      resource_domains: ['https://cdn.sunpeak.ai'],
    },
  }
}
```
