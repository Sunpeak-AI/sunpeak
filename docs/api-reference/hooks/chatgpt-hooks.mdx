...
These hooks wrap the ChatGPT `window.openai` runtime APIs, providing typed React hooks for host-specific features that go beyond the MCP Apps standard. They are available **only when running inside ChatGPT** (or the simulator with the ChatGPT host selected).
