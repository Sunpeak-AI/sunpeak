...
| `--live` | | Start with the simulator's **Live** mode enabled (real tool handlers). Can also be toggled from the sidebar at runtime. |
...
| `--live` | HMR | Real handlers | Integration testing with real backend logic |
| `--built` | Production bundle | Mocked | Catch build regressions, CI/E2E testing |
| `--live --built` | Production bundle | Real handlers | Final smoke test (simulator shell around production behavior) |
...
sunpeak dev --live --built
```

When `--built` is used, `sunpeak build` runs automatically before the dev server starts. The built HTML is served from `dist/`.
