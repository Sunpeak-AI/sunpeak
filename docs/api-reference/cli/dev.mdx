...
|--------|-------|-------------|
| `--port <number>` | `-p` | Port for the simulator (default: 6767, also respects `PORT` env var) |
| `--prod-mcp` | | Serve production build `.js` files over MCP instead of Vite HMR (requires `sunpeak build` first) |
