...
3. Uploads each resource bundle to the Sunpeak repository
...
```bash
$ sunpeak push -t v1.0.1
Pushing 2 resource(s) to repository "Sunpeak-AI/sunpeak"...
Tags: v1.0.1

✓ Pushed albums, 1 simulation(s), tag(s): v1.0.1
  https://app.sunpeak.ai/resources/68092b94-dbe0-4028-b6a8-00735a954e58
✓ Pushed carousel, 1 simulation(s), tag(s): v1.0.1
  https://app.sunpeak.ai/resources/9370f050-a875-47b5-a8f2-652894d79a73
...
| `--no-simulations` | | Skip pushing simulations, only push resource bundles |
...
Each resource folder needs an `.html` bundle and a `.json` metadata file containing name, title, description, and uri.
URIs are generated during `sunpeak build`--if a resource is missing its URI, push will fail with an error.
Simulation files are automatically included when pushing, but can be omitted with `--no-simulations`.
