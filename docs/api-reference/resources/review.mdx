...
The resource uses `useToolData(app).output` for configuration data and `useAppState(app)` to persist the user's decision across sessions.

## Data Structure

### ReviewData

The resource expects data in the following format via `useToolData(app).output`:
...
The resource uses `useAppState(app)` to persist the user's decision:
...
Includes a fullscreen toggle button using `useDisplayMode(app)` and `useCallServerTool(app)` for expanded viewing of complex reviews.
...
Respects device safe areas using `useSafeArea(app)` to ensure proper padding on devices with notches or rounded corners.

### Height Constraints

Uses `useViewport(app)` to adapt to available screen space and prevent overflow.
