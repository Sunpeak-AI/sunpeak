...
The resource retrieves album data from `useToolData(app).output` and manages selection state with `useAppState(app)`.
...
1. Sets the selected album ID in app state using `useAppState(app)`.
2. Requests fullscreen mode via `useDisplayMode(app)`.
...
The resource uses `useAppState(app)` to persist the selected album across sessions:
