...
The resource retrieves place data from `useToolData(app).output` and manages selection state with `useAppState(app)`.
...
1. Requests fullscreen mode via `useDisplayMode(app)`.
...
The resource uses `useAppState(app)` to persist the selected place across sessions:
