...
<Note>This documentation covers MCP Apps SDK v1.2.2 (`@modelcontextprotocol/ext-apps`).</Note>
...
| `AppNotification` | Union of all notification types in both directions: Host → View (`toolInput`, `toolInputPartial`, `toolResult`, `toolCancelled`, `hostContextChanged`) and View → Host (`initialized`, `sizeChanged`) |
