...
If you are wiring the server side from scratch, start with [Build an MCP App Server](/mcp-apps/server/build-server). It shows capability detection, `ui://` resources, `_meta.ui.resourceUri`, `structuredContent`, CSP, and app-only tools in one example. For a field-by-field check before testing in a host, use the [Tool and Resource Contract](/mcp-apps/server/tool-resource-contract).
