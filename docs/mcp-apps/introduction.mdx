...
<Note>This documentation covers MCP Apps SDK v1.7.3 (`@modelcontextprotocol/ext-apps`).</Note>
...
The [Patterns](/mcp-apps/patterns) guide covers app-only tools, polling, large data, errors, and host adaptation. The [MCP Apps FAQ](/mcp-apps/faq) answers common questions about hosts, UI delivery, display modes, theming, security, and testing.

If you are wiring the server side from scratch, start with [Build an MCP App Server](/mcp-apps/server/build-server). It shows capability detection, `ui://` resources, `_meta.ui.resourceUri`, `structuredContent`, CSP, and app-only tools in one example.
