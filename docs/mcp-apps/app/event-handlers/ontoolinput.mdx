...
The sunpeak framework provides the [`useToolData`](/api-reference/hooks/use-tool-data) hook, which wraps `ontoolinput` (along with related handlers) into a reactive value for React components. This is the recommended approach when building with sunpeak.
