...
<Tip>
Register event handlers in the `onAppCreated` callback of [`useApp`](/mcp-apps/react/use-app) to ensure they're set before the handshake. The [sunpeak framework](/quickstart) provides convenience hooks: [`useToolData`](/api-reference/hooks/use-tool-data), [`useTeardown`](/api-reference/hooks/use-teardown), and [`useAppTools`](/api-reference/hooks/use-app-tools).
</Tip>
