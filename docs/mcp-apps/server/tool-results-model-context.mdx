...
MCP App data crosses three boundaries: the model reads the tool result, the View renders the UI, and the host manages the iframe and conversation state. Use the right field for each boundary so text-only MCP clients keep working and the View does not scrape text.

## Field Guide

| Field                | Set by                                 | Read by                              | Use it for                                                                   |
| -------------------- | -------------------------------------- | ------------------------------------ | ---------------------------------------------------------------------------- |
| `content`            | Tool handler                           | Model, host, logs, text-only clients | Short human-readable summary and recoverable errors.                         |
| `structuredContent`  | Tool handler                           | View and compatible clients          | Typed data the UI renders, matching `outputSchema` when one is declared.     |
| `_meta`              | Tool handler or resource read callback | Host and app runtime                 | Opaque metadata, IDs, hints, CSP, iframe permissions, or host-specific data. |
...
flowchart LR
  ...
  Result --> Structured["structuredContent: View data"]
