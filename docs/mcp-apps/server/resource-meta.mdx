...
Set `_meta.ui` on individual `contents[]` items returned by the [`registerAppResource`](/mcp-apps/server/register-app-resource) read callback when the host needs to enforce CSP, permissions, stable domains, or border preferences for the rendered View.
...
<Tip>
In the [sunpeak framework](/quickstart), resource metadata is co-located with resource components. See the [Tool File Reference](/app-framework/tools/tool-file).
</Tip>
