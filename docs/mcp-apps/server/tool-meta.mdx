...
| `"app"` | Tool is callable by the View (your UI) via [`app.callServerTool()`](/mcp-apps/app/requests#callservertool) on the same server connection |
...
<Tip>
The [sunpeak framework](/quickstart) provides [`useAppTools`](/api-reference/hooks/use-app-tools) for calling app-visible tools from React components, and the [Tool File Reference](/api-reference/tools/tool-file) for defining tool metadata.
</Tip>
