...
Must match a URI registered via [`registerAppResource`](/mcp-apps/server/register-app-resource). When the host invokes this tool, it calls `resources/read` with this URI to fetch the HTML content, then renders it in a sandboxed iframe.
...
| `"app"` | Tool is callable by the View (your UI) via [`useAppTools`](/mcp-apps/app/use-app-tools) on the same server connection |
