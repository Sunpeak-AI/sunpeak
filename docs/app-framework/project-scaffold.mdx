...
- `tests/unit/` - Directory containing unit tests for component and hook logic (Vitest + happy-dom).
...
- `tests/evals/` - Directory containing multi-model eval specs and config.
...
Automated testing against real ChatGPT. Tests live in `tests/live/`. Run with `pnpm test:live`. See the [Testing Guide](/testing/live) for setup details.
...
<Card horizontal title="sunpeak CLI API Reference" icon="code" href="/app-framework/cli/new">
