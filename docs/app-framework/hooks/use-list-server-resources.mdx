...
Returns a function to discover available resources on the originating MCP server. The request is proxied through the host. Supports pagination via cursor. Use [`useReadServerResource`](/app-framework/hooks/use-read-server-resource) to read a discovered resource.

<Card horizontal title="Wraps listServerResources" icon="cube" href="/mcp-apps/app/requests/list-server-resources">
